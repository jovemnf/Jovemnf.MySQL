...
public class UpdateQueryBuilder
{
    ...
    public UpdateQueryBuilder WhereIn(string field, IEnumerable<object> values)
    {
        _whereConditions.Add(new WhereCondition 
        { 
            Field = field, 
    ...
    public UpdateQueryBuilder WhereNotIn(string field, IEnumerable<object> values)
    {
        _whereConditions.Add(new WhereCondition 
        { 
            Field = field, 
    ...
    public (string Sql, MySqlCommand Command) Build()
    {
        ...
        if (_whereConditions.Count == 0)
            throw new InvalidOperationException("Nenhuma condição WHERE definida");
        ...
        var sql = $"UPDATE `{EscapeIdentifier(_tableName)}` SET {string.Join(", ", setClauses)} WHERE {string.Join(" ", whereClauses)}";
